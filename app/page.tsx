...
export default function ProcurementDashboard() {
  ...
  const handleAutoFillPrices = async (scope: 'all' | 'non-selected' | 'selected') => {
    ...
    if (projectId && changed.length > 0) {
      ...
      toast({
        title: 'Prices saved',
        description: `${changed.length} item${changed.length > 1 ? 's' : ''} converted to project currency.`,
      })
    }
  }
  ...
  const handleExecuteAction = () => {
    ...
    // Preserve main's select-all behavior while treating "No Action" as an
    ...
    const missingActionItems = items.filter((it: any) => {
      const action = String(it.action || '').trim()
      return action.toLowerCase() !== 'no action' && !NORMALIZED_ACTIONS[action]
    })

    if (actionableItems.length === 0) {
      const onlyNoAction = noActionItems.length === items.length
      toast({
        title: onlyNoAction ? 'No actions to execute' : 'No actionable items',
        description: onlyNoAction
          ? `${noActionItems.length} selected item(s) are assigned No Action. Nothing was created.`
          : 'None of the selected items have a valid action assigned. Run Assign Actions first.',
        variant: onlyNoAction ? undefined : 'destructive',
      })
      return
    }

    const skippedCount = noActionItems.length + missingActionItems.length
    if (skippedCount > 0) {
      toast({
        title: `Skipped ${skippedCount} non-actionable item(s)`,
    ...
    // picker) operates only on actionable rows.
  ...
  // Add identifier, custom ID and spec columns dynamically after itemId, description, mpn
  // (kept after description so the first two sticky columns stay Item ID + Description).
  // MPN already has its own "mpn" column, so it's excluded here to avoid a duplicate.
  const identifierColumnKeys = IDENTIFIER_COLUMNS.map(c => c.key).filter(key => key !== 'mpn_item_code')
  ...
  // Add identifier column labels + default widths (MPN uses the existing "mpn" column)
  IDENTIFIER_COLUMNS.forEach(({ key, label }) => {
    if (key === 'mpn_item_code') return
                  ...
                  <button
                    onClick={() => {
                      if (displayCurrency !== 'native') {
                        ...
                        toast({ title: 'Currency: Native', description: 'Showing each source in its original currency.' })
                      }
                    }}
                  ...
                  <button
                    onClick={() => {
                      if (displayCurrency !== 'project') {
                        setDisplayCurrency('project')
                        toast({ title: 'Currency: Project', description: 'All prices converted to item currency (standard rate).' })
                      }
                    }}
